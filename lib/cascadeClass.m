...
%
  ...
  methods
    ...
    function gn = scaleFltr(obj, wp)
      ...
      gprdct = ones(size(f));
      for i_ = 1:obj.size
        g = obj.sctns(i_).freqEval(f);
        gprdct = gprdct.*g;
        currentGn = max(abs(gprdct));
        gnChange = 1/currentGn;
        obj.sctns(i_).setK(obj.sctns(i_).k * gnChange);
    ...
    function sys = fshftFltr(obj, fshft)
      for i_ = 1:obj.size
        obj.sctns(i_).sys = freq_shiftd(obj.sctns(i_).sys, fshft);
      end
      sys = getSystem(obj);
    end
    ...
    function sys = fsclFltr(obj, sclFctr)
      for i_ = 1:obj.size
        obj.sctns(i_).sys = scaleFltrD(obj.sctns(i_).sys, sclFctr);
      end
      sys = getSystem(obj);
    end
    ...
    function out = sim(obj, xin, delta_f)
      ...
      xo = xin;
      for i_ = 1:obj.size
        xo = obj.sctns(i_).sim(xo, delta_f);
      end
      out = xo;
    end
