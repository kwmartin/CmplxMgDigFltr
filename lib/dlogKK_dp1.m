function h = dlogKK_dp1(sys,px,s)
%   h = dlogKK_dp1(sys,px,s) is a program for calculating the derivative
%   of the log of the magnitude of system Kz_ with respect to the loss poles.
%   sys: the LTI object.
%   s: the frequencies of the minima
%   h: the returned matrix has one row for each minima frequency and one
...
for i_ = 1:ls % For each frequency
  p2 = pz2 - s(i_)^2; % Calculate row vector
  if any(p2==0) % Don't divide by zero
	 h(i_,:) = Inf;
  else
	 h(i_,:) = -4*(p_./p2); % Calcuate row of derivatives of ln(Kz_*Kz_')
  end
end
